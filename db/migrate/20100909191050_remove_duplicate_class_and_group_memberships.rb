class RemoveDuplicateClassAndGroupMemberships < ActiveRecord::Migration
  def self.up
    count = NodeClassMembership.destroy_all('id NOT IN (SELECT min(id) FROM node_class_memberships GROUP BY node_id, node_class_id)').count
    if count > 0
    ...
    count = NodeGroupMembership.destroy_all('id NOT IN (SELECT min(id) FROM node_group_memberships GROUP BY node_id, node_group_id)').count
    if count > 0
    ...
    count = NodeGroupClassMembership.destroy_all('id NOT IN (SELECT min(id) FROM node_group_class_memberships GROUP BY node_group_id, node_class_id)').count
    if count > 0
    ...
    count = NodeGroupEdge.destroy_all('id NOT IN (SELECT min(id) FROM node_group_edges GROUP BY to_id, from_id)').count
    if count > 0
