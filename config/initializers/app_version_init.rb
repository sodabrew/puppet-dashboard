...
# will have a CHANGELOG file that is generated at tarball
# build time.
#
# This allows for the APP_VERSION to be determined by git, so
# developers can run out of master after a git checkout.
def get_app_version
  if File.exists?(Rails.root.join('CHANGELOG'))
    return File.open(Rails.root.join('CHANGELOG')) {|f| f.readline}.strip
